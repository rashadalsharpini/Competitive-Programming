...
def solve():
    ...
    friends = []
    for i in range(n):
        dump = list(input().split())
        name = dump[0]
        a, m, s, e = map(int, dump[1:])
        friends.append([name, a+m+s+e, a, m, s, e])
    friends_sorted = sorted(friends, key=lambda x: [-x[1], x[0]])
    # print(*friends_sorted, sep = '\n')
    for i in range(n):
